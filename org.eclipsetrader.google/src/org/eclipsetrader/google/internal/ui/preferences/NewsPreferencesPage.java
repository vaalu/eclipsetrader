...
package org.eclipsetrader.google.internal.ui.preferences;
...
import org.eclipsetrader.google.internal.news.Category;
import org.eclipsetrader.google.internal.news.Page;
...
public class NewsPreferencesPage extends PreferencePage implements IWorkbenchPreferencePage {
    ...
    protected void performDefaults() {
        ...
        interval.setSelection(store.getInt(GoogleActivator.PREFS_NEWS_UPDATE_INTERVAL));
        hoursAsRecent.setSelection(store.getInt(GoogleActivator.PREFS_HOURS_AS_RECENT));
        updateNews.setSelection(store.getBoolean(GoogleActivator.PREFS_UPDATE_SECURITIES_NEWS));
        ...
        try {
            ...
            for (Category category : element.getValue()) {
                ...
                boolean someChecked = false;
                for (Page page : category.getPages()) {
                    boolean checked = store.getBoolean(GoogleActivator.PREFS_SUBSCRIBE_PREFIX + page.getId());
                    someChecked = someChecked || checked;
                    allChecked = allChecked && checked;
            ...
            Status status = new Status(IStatus.WARNING, GoogleActivator.PLUGIN_ID, 0, "Error reading feed sources", e); //$NON-NLS-1$
    ...
    public boolean performOk() {
        ...
        store.setValue(GoogleActivator.PREFS_NEWS_UPDATE_INTERVAL, interval.getSelection());
        store.setValue(GoogleActivator.PREFS_HOURS_AS_RECENT, hoursAsRecent.getSelection());
        store.setValue(GoogleActivator.PREFS_UPDATE_SECURITIES_NEWS, updateNews.getSelection());

        Category[] input = (Category[]) providers.getInput();
        for (Category category : input) {
            for (Page page : category.getPages()) {
                store.setValue(GoogleActivator.PREFS_SUBSCRIBE_PREFIX + page.getId(), providers.getChecked(page));
            }
        }
